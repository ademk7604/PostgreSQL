...
select urun_adi, min(urun_miktari) as minimum_urun_miktari, max(urun_miktari) as maksimum_urun_mik from manav
where urun_adi is not null -- null olanida cikardik
group by urun_adi
order by maksimum_urun_mik -- sonucta tabloda buraya gore siraladi, tabi min de verdi ama siralamayi maxz a gore yaptik
...
select ulke, round(avg(maas)) as ortalama_maas  -- as ile isimde verdik stunumuz isimlendirildi
...
from personel group by ulke,sehir
...
select ulke, cinsiyet, round(avg(yas)) as ortalama_yas, count(*) from personel -- yildiz herikisi icin saydir gruplandir
where maas>30000
group by ulke, cinsiyet
...
select ulke, cinsiyet, round(avg(yas)) as ortalama_yas, count(*) from personel // count(*) herseyi sayiyor burda
where maas>30000
group by ulke, cinsiyet
