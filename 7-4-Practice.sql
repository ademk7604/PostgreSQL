...
-- in ile bagliyoruz. isyeri ortak paydamiz iki tablonun o yuzden girdik/
select isim, maas, isyeri from calisanlar -- ikinci cozumdur. isyeri bus atira girmayebilir ama tablosunu gormek istedigimden girdik
where isyeri in(select marka_isim from markalar where calisan_sayisi>15000); 
-- in kullanacaksaniz parantez icine aldiginiz sorgu birden fazla deger dondurmeli

select * from calisanlar; -- tablo icin birlesme yeri isyeri stunu
select * from markalar; -- tablosu ucun birlesme yeri marka_isim stunu
